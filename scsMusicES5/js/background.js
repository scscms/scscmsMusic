...
if (typeof Promise.prototype.finally !== 'function') {
    ...
    var shim = {
            ...
            return Promise.prototype.then.call(promise, x => new C(resolve => resolve(onFinally())).then(() => x), e => new C(resolve => resolve(onFinally())).then(() => {
                throw e;
            }));
        }
    };
...
function operation(action, val) {
    ...
    switch (action) {
        ...
        case 'setVolume':
            ...
            break;
        case 'mute':
            (obj.mute = !obj.mute) ? obj.analyser.disconnect(gainNode) : obj.analyser.connect(gainNode);
            break;
        default:
    }
}
...
chrome.contextMenus.removeAll(() => {
    menuId = addMenus('播放', function () {
        operation('playing');
    });
    addMenus('上一曲', function () {
        operation('prior');
    });
    addMenus('下一曲', function () {
        operation('next');
    });
